...
import { deleteVideoProgress, getVideoProgress, saveVideoProgress, getVideo } from '../api';
...
async function reloadVideo() {
	showVideoRetry = false;
	const newVideo = await patchYoutubeJs(data.video.videoId);
	data.video = newVideo;
	data.content = phaseDescription(newVideo.videoId, newVideo.descriptionHtml, newVideo.fallbackPatch);
	await loadVideo();
}

// Function to load new video data without page navigation
async function loadNewVideo(videoId: string) {
	try {
		const newVideoData = await getVideo(videoId);
		
		// Store fullscreen state
		const wasFullscreen = !!document.fullscreenElement;
		
		// Update the data object to trigger reactivity
		data.video = newVideoData;
		data.content = phaseDescription(newVideoData.videoId, newVideoData.descriptionHtml, newVideoData.fallbackPatch);
		
		// Update browser URL without navigation
		const newUrl = `/${$isAndroidTvStore ? 'tv' : 'watch'}/${videoId}${data.playlistId ? `?playlist=${data.playlistId}` : ''}`;
		history.replaceState(null, '', newUrl);
		
		// Update document title
		document.title = `${newVideoData.title} | BluTube`;
...
onMount(async () => {
	...
	// MODIFIED: This is the key change for fullscreen persistence
	playerElement.addEventListener('ended', async () => {
		...
		if (typeof goToVideo !== 'undefined') {
			...
			// Try to load new video without navigation first
			const success = await loadNewVideo(goToVideo.videoId);
			
			// If that fails, fallback to navigation
			if (!success) {
				goto(
